...
keymap.set(
  ...
  'to',
  ':tabo<Return>',
  { silent = true, desc = 'Close all tabs but current' }
)
...
keymap.set(
  ...
  'si',
  ':split<Return><C-w>w',
  { silent = true, desc = 'Horizontally split current buffer' }
)
keymap.set(
  'n',
  'sv',
  ':vsplit<Return><C-w>w',
  { silent = true, desc = 'Vertically split current buffer' }
)
...
keymap.set(
  ...
  '>',
  '>gv',
  { noremap = true, silent = true, desc = 'Indent selected text' }
)
keymap.set(
  'v',
  '<',
  '<gv',
  { noremap = true, silent = true, desc = 'Unindent selected text' }
)
...
keymap.set(
  ...
  'K',
  ":move '<-2<CR>gv-gv",
  { noremap = true, silent = true, desc = 'Move selected text up' }
)
keymap.set(
  'x',
  'J',
  ":move '>+1<CR>gv-gv",
  { noremap = true, silent = true, desc = 'Move selected text down' }
)
